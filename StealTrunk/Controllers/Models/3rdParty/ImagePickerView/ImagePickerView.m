...
- (void)clickPhoto:(NSInteger)sender
{
    ...
    if (sender == 0) {
        ...
        if (![UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeCamera]) {
            ...
            UIView *aView = [[UIView alloc] initWithFrame:CGRectMake(0, 54, 320, 320)];
            aView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.5];
            pickerController.cameraOverlayView = aView;
            */
...
{    
    ...
    if (_delegate != nil && [_delegate respondsToSelector: @selector(setViewPhoto:sender:)]) {    //不需要保存照片的类都相应此调用
        [_delegate performSelector: @selector(setViewPhoto:sender:) withObject: path withObject: tapSender];
    }else if (_delegate != nil && [_delegate respondsToSelector: @selector(setViewPhotoInfo:sender:)]) { //需要保存照片的类都响应此调用
        [_delegate performSelector: @selector(setViewPhotoInfo:sender:) withObject: info withObject: tapSender];
    }
}
