...
@interface KLSwitchThumb : UIImageView
...
-(void) configureSwitch {
    ...
    if (!_track) {
        if(self.onImage)
        {
            _track = [[KLSwitchTrack alloc] initWithFrame: CGRectMake(0, 0, 60, 20)//self.bounds
                                              onColor: self.onTintColor
                                             offColor: self.tintColor
                                        contrastColor: self.contrastColor];
            self.bounds = CGRectMake(0, 0, 64, 24);
        }
        else
        {
    ...
    if (!_thumb) {
        if(self.onImage)
        {
            _thumb = [[KLSwitchThumb alloc] initWithFrame:CGRectMake(-4, -4, 30, 30)];
        }
        else
        {
...
- (void)drawRect:(CGRect)rect
{
    ...
    if(self.onImage)
    {
        [_thumb setBackgroundColor: [UIColor clearColor]];
        [_thumb setImage:self.onImage];
        [_thumb.layer setBorderWidth: 0.];
        [_thumb.layer setBorderColor: [UIColor clearColor].CGColor];
        [_thumb.layer setShadowColor: [[UIColor clearColor] CGColor]];
...
-(void) setThumbOn:(BOOL) on
    ...
    if (on) {
        ...
        thumbFrame.origin.x = kThumbOffset - (self.onImage? 4:0);
    }
