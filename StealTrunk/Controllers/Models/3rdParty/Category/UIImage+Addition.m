...
//  Copyright (c) 2012年 StealTrunk.com inc. All rights reserved.
//
...
//八对
- (UIImage *)imageCropCenter
{
    float cropSize = 0;
    
    if ((self.size.width/self.size.height)>1) {
        cropSize = self.size.height;
    }else if((self.size.width/self.size.height)<1){
        cropSize = self.size.width;
    }else {
        return self;
    }
    
    CGRect cropRect = CGRectMake((self.size.width-cropSize)/2, (self.size.height-cropSize)/2, cropSize, cropSize);
    
    //create drawing context
	UIGraphicsBeginImageContextWithOptions(cropRect.size, NO, 0.0f);
...
+ (UIImage *)addImage:(UIImage *)image1 toImage:(UIImage *)image2
{
    ...
    //Draw image1
    [image1 drawInRect:CGRectMake(50, 50, image1.size.width, image1.size.height)];
...
{
    CGRect squareRect = CGRectMake(0, (image.size.height-image.size.width)/2, image.size.width, image.size.width);
