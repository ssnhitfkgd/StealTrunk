...
//		[self presentAlertWithTitle:NSLocalizedStringFromTable(@"提示", @"提示", nil)
	...
	@synchronized(self)
	{
		...
		if (fileLength > 0 && seekByteOffset > 0)
		{
			CFHTTPMessageSetHeaderFieldValue(message, CFSTR("Range"),
				(__bridge CFStringRef)[NSString stringWithFormat:@"bytes=%ld-%ld", seekByteOffset, (long)fileLength]);
			discontinuous = YES;
		}
